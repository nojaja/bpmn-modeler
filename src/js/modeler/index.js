...
async function saveSVG() {
  //svgのタグにcontent属性を追加して編雁E��チE�Eタをbase64で埋め込む
...
async function handleSavePNGClick(event) {
  ...
  try {
    ...
    const compressed = await zip.generateAsync({type:'arraybuffer', compression: "DEFLATE",compressionOptions: {level: 1} }) //blobタイプ�Ezipを作�E、E
...
async function importXML(contents) {
  //svgの場合�EsvgタグのcontentからチE�Eタを取り�EぁE
